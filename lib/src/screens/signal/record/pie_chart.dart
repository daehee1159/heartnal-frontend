...
class PieChart extends CustomPainter {
  ...
  PieChart({required this.percentage, required this.textScaleFactor, required this.textColor});
  ...
  double arcAngle = -2 * pi * (percentage / 20);
  // 호를 그릴 때는 색을 바꿔줌.
  paint..color = const Color(0xffFE9BE6);
  // 호(arc)를 그림.
  canvas.drawArc(Rect.fromCircle(center: center, radius: radius), -pi / 2, arcAngle, false, paint);
  // 텍스트를 화면에 표시함.
  drawText(canvas, size, "$percentage / 20");
  }

  // 원의 중앙에 텍스트를 적음.
  void drawText(Canvas canvas, Size size, String text) {
  double fontSize = getFontSize(size, text);
  // TextSpan 은 Text 위젯과 거의 동일하다.
