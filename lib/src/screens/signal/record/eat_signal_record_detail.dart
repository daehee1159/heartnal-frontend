...
int _memberSeq = 0;
String datetime = "";
List<RecentSignal> eatSignalList = [];
...
class EatSignalRecordDetail extends StatefulWidget {
  const EatSignalRecordDetail({Key? key}) : super(key: key);

  @override
  State<EatSignalRecordDetail> createState() => _EatSignalRecordDetailState();
}

class _EatSignalRecordDetailState extends State<EatSignalRecordDetail> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    List<RecentSignal> allEatSignalList = [];
    return Scaffold(
      appBar: AppBar(
        title: Text(
          '오늘 뭐먹지 이력보기',
        ...
        child: Container(
          ...
          child: Column(
            children: [
                    ...
                    FutureBuilder(
                      ...
                      builder: (BuildContext context, AsyncSnapshot snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          _eatSignalRecentCount = _recentSignal.getEatSignalRecentCount;
                          _eatSignalRecentSuccessCount = _recentSignal.getEatSignalRecentSuccessCount;
                          return Column(
                            children: [
                              Center(
                                child: Column(
                                  children: [
                                    Container(
                                      child: CustomPaint(
                                        ...
                                        painter: PieChart(
                                          percentage: _eatSignalRecentSuccessCount,
                                          textScaleFactor: 1.0,
              ...
              Divider(),
              FutureBuilder(
                future: _recentSignal.getAllSignalList('eatSignal'),
                builder: (BuildContext context, AsyncSnapshot snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return Container(
                      child: Center(
                    ...
                    allEatSignalList = snapshot.data;
                    if (allEatSignalList.isEmpty || snapshot.data == null) {
                      _isEmpty = true;
                    }
                    eatSignalList = snapshot.data;
                    ...
                    for (var i = 0; i < allEatSignalList.length; i++) {
                      int diff = now.difference(DateTime.parse(allEatSignalList[i].regDt.toString().substring(0, 10))).inDays;

                      if (-1 < diff && diff < 8) {
                        oneWeekList.add(allEatSignalList[i]);
                        oneMonthList.add(allEatSignalList[i]);
                        threeMonthList.add(allEatSignalList[i]);
  ...
  _cardView(BuildContext context, List<RecentSignal> list, String period ) {
    ...
    if (list.length == 0) {
      ...
      return FutureBuilder(
          future: _recentSignal.getMostMatchedSignalItem('eatSignal', list.first.regDt.toString().substring(0, 10), list.last.regDt.toString().substring(0, 10)),
          builder: (BuildContext context, AsyncSnapshot snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return Container(
                child: Center(
